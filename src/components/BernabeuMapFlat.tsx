...
// ── Single Section (Line + pre-computed label) ─────────
const SectionWithLabel: React.FC<{
  ...
  // Blocks whose id starts with "0" or "technical" are structural
  const isTechnical =
    section.id.startsWith("0") || section.id.startsWith("technical");

  const labelOffsetX = (section.label?.length || 0) * 4.5;

  return (
    <Group>
      {/* ── The shape: flat point array, no SVG strings ── */}
      <Line
      ...
      />
      {/* ── Pre-computed label position (no getSelfRect needed!) ── */}
...
export default function BernabeuMapFlat({
  activeBlockId,
  hoveredBlockId,
  setHoveredBlock,
  setSelectedBlock,
